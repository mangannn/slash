...
void DummyControls::eventHandle(sf::Event event) {

	if (clock.getElapsedTime().asSeconds() > 2.0) {
		clock.restart();
		controlled->eventCallback(type);
	}
}
bool DummyControls::action(int num) {return false;}
Vector2f DummyControls::movement() {return Vector2f(0,0);}
